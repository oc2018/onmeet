    ...
    return (
        ...
        {calls && calls.length > 0 ? calls.map((meeting: Call | CallRecording) =>
         (
            ...
            title={
                (meeting as Call)?.state?.custom?.description?.substring(0, 26) || (meeting as CallRecording)?.filename?.substring(0, 26) || 'Personal Meeting'
            }
            date={
            ...
            handleClick={
                type === 'recordings' ? () => router.push(`${(meeting as CallRecording).url}`): () => router.push(`/meetings/${(meeting as Call).id}`)
            } 
            link={
                type === 'recordings' ? (meeting as CallRecording).url : `${process.env.NEXT_PUBLIC_BASE_URL}/meeting/${(meeting as Call).id}`
            }
            />
            )):(<h1 className='text-2xl font-bold text-sky-1'>{noCallsMessage}</h1> )
        }
    </div>
  );
};
